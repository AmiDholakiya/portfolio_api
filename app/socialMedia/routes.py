...
async def index(pageSize: int = 10, pageNumebr: int = 1, login_data = Depends(validateToken)):
    try:
        ...
        resultList = list()
        total_document = socialMedial_col.count_documents({"user_id":login_data["_id"]})
        get_all = socialMedial_col.find({"user_id":login_data["_id"]},{"user_id":0,"created_at":0}).skip(skips).limit(pageSize).sort("created_at",1)
        for item in get_all:
...
async def get_socialMedia_ById(id:str,login_data = Depends(validateToken)):
    try:
        result = socialMedial_col.find_one({"_id": ObjectId(id),"user_id":login_data["_id"]},{"user_id":0,"created_at":0})
...
@router.put("/{id}")
async def update_socialMedia(id:str, item:SocialMediaUpdateModel = Depends(), file: UploadFile = File(None) ,login_data = Depends(validateToken) ):
    try :
        old_object = socialMedial_col.find_one({"_id": ObjectId(id), "user_id":login_data["_id"]})
        ...
        if len(socialMedia) >= 1:
            update_result = socialMedial_col.update_one({"_id": ObjectId(id),"user_id":login_data["_id"]}, {"$set": socialMedia})
...
async def delete_socialMedia(id:str, login_data = Depends(validateToken)):
    try:
        delete_result = socialMedial_col.find_one_and_delete({"_id": ObjectId(id), "user_id":login_data["_id"]})
